...
import { Plus, Copy, ExternalLink, Trash2, X, LogOut } from 'lucide-react'
...
export default function Dashboard() {
  ...
  const handleCreate = async (data) => {
    ...
    if (id) navigate(`/dashboard/form/${id}`)
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', paddingBottom: 80, position: 'relative', overflow: 'hidden' }}>
      <Blobs />
      <GridLines />
      <motion.div animate={{ scale: [1, 1.1, 1], opacity: [0.08, 0.14, 0.08] }} transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut' }}
      ...
      <div style={{ borderBottom: '1px solid var(--border)', padding: '18px 48px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(237,234,228,0.9)', backdropFilter: 'blur(20px)', position: 'sticky', top: 0, zIndex: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <span className="tag" style={{ background: 'var(--blue)', color: 'white', padding: '2px 8px', borderRadius: 2 }}>DASHBOARD</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <span className="tag">{user?.email}</span>
          <button onClick={handleSignOut} style={{ background: 'transparent', border: '1px solid var(--border-strong)', borderRadius: 'var(--radius)', padding: '7px 12px', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', fontSize: 11, fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
            <LogOut size={12} /> Sair
          </button>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 64, paddingBottom: 40, borderBottom: '1px solid var(--border)' }}>
          <div>
            ...
            <motion.h1 {...fade(0.1)} style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(48px, 6vw, 80px)', fontWeight: 400, lineHeight: 0.9, letterSpacing: '0.02em' }}>
              MEU<br /><span style={{ color: 'var(--blue)' }}>DASHBOARD</span>
            </motion.h1>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 1, background: 'var(--border)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
          <AnimatePresence>
            {loading ? (
              ...
              <motion.div key={form.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, x: -20 }} transition={{ delay: i * 0.05 }}
                ...
                <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                  <button onClick={() => navigate(`/dashboard/form/${form.id}`)}
